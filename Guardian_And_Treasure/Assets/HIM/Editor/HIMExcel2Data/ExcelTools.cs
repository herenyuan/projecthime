...
public class ExcelTools : EditorWindow
{
    ...
    private static List<string> excelFileName;
	/// <summary>
	/// 项目根路径	
	/// </summary>
	...
	private void DrawOptions()
	{
        ...
        string newPath = EditorGUILayout.TextField(ExcelPath);
        if (!ExcelPath.Equals(newPath))
        {
            ExcelPath = newPath;
            Debug.Log("路径输入：" + ExcelPath);
        }
        GUILayout.Button("导入配置");
	...
	private static void Convert()
	{
		foreach(string assetsPath in excelList)
		{
            ...
            string[] name = assetsPath.Split('/');
            string savePath = Application.dataPath+"/HOA/Resources/Config/"+ name[name.Length - 1];
            Debug.Log(savePath);
			//构造Excel工具类
			...
			string output="";
			if(indexOfFormat==0){
				output= savePath.Replace(".xlsx",".json");
